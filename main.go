...
func main() {
	...
	captchaSolveMode := flag.String("captcha-solve-mode", "off", "yandex/yandex_multistream only: 'off' waits for cookies/cooldown, 'native' enables the experimental PoW solver, or 'headless_browser' uses Chrome/Chromium from PATH (see README)")
...
func parseCaptchaSolveMode(s string) yandex.CaptchaSolveMode {
	...
	case "native":
		return yandex.CaptchaSolveModeNative
	default:
